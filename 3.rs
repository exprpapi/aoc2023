...
fn main() {
  assert_eq!(sol_part1(INPUT_EXAMPLE), 4361);
  assert_eq!(sol_part1(INPUT), 540025);
  assert_eq!(sol_part2(INPUT_EXAMPLE), 467835);
  assert_eq!(sol_part2(INPUT), 84584891);
}

fn sol_part1(input: &str) -> u32 {
  let a = input.lines().map(|line| line).collect::<Vec<_>>();
  let (nrows, ncols) = (a.len(), a[0].len());
  let has_symbol = |row: usize, begin: usize, end: usize| {
    let (row_before, row_after) = (row.saturating_sub(1), (row + 1).min(nrows.saturating_sub(1)));
    let (col_before, col_after) = (begin.saturating_sub(1), end.min(ncols.saturating_sub(1)));
    let is_symbol = |i: usize, j: usize| {
      let c = a[i][j..j+1].chars().next().unwrap();
      !c.is_digit(10) && c != '.'
    };
    (col_before..=col_after).any(|col| is_symbol(row_before, col) || is_symbol(row_after, col))
      || is_symbol(row, col_before)
      || is_symbol(row, col_after)
  };
  ...
  (0..nrows)
    .map(|row|
      re
        .find_iter(a[row])
        .filter(move |m| has_symbol(row, m.start(), m.end()))
    )
    .flatten()
...
fn sol_part2(input: &str) -> u32 {
  ...
  let gears = RefCell::new(HashMap::<(usize, usize), Vec<u32>>::new());
  let a = input.lines().map(|line| line).collect::<Vec<_>>();
  let (nrows, ncols) = (a.len(), a[0].len());
  let insert_gears = |row: usize, m: regex::Match| {
    let number = m.as_str().parse::<u32>().unwrap();
    let (begin, end) = (m.start(), m.end().saturating_sub(1));
    let (row_before, row_after) = (row.saturating_sub(1), nrows.saturating_sub(1).min(row + 1));
    let (col_before, col_after) = (begin.saturating_sub(1), ncols.saturating_sub(1).min(end + 1));
    let insert_if_gear = |i: usize, j: usize| {
      if a[i][j..j+1] == *"*" {
        gears
          .borrow_mut()
          .entry((i, j))
          .and_modify(|existing| existing.push(number))
          .or_insert(vec![number]);
      }
    };
    for col in col_before..=col_after {
      insert_if_gear(row_before, col);
      insert_if_gear(row_after, col);
    }
    insert_if_gear(row, col_before);
    insert_if_gear(row, col_after);
  };
  let re = regex::Regex::new(r"\d+").unwrap();
  (0..nrows).for_each(|row| re.find_iter(a[row]).for_each(|m| insert_gears(row, m)));
  let sum = gears.borrow().values().filter(|v| v.len() == 2).map(|v| v.iter().product::<u32>()).sum();
  sum
}
